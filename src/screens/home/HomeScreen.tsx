...
//     return `BPT-${segment(8)}-${segment(4)}-${segment(4)}-${segment(4)}`;
//   };
...
const HomeScreen: React.FC<HomeScreenProps> = ({ navigation }) => {
  ...
  const actionBoxSize = Math.round((isLandscape ? 56 : 68) * Math.max(0.7, Math.min(1.2, rs)));
  ...
  const StatusGauge = () => {
    ...
    const containerSize = isLandscape ? size : size + 40;
    ...
    return (
      <View style={[styles.gaugeWrapper, { width: containerSize, height: containerSize, alignItems: 'center', flex: 1, justifyContent: 'center', }]}>
        ...
        <Image
          source={gaugeImage}
          style={{ width: size + 40, height: size, resizeMode: 'contain' }}
        />
      </View>
    );
  };
  ...
  return (
    <>
      ...
      >
        ...
        {!isPortrait && (
          <View style={[styles.gpsRow, { bottom: isMobile && isLandscape ? insets.bottom : insets.bottom + 5, flex: isTablet ? 0.1 : 0.2, paddingRight: 15 }]}>
