...
//     // Monitor TTS finish to auto-close modal
//     useEffect(() => {
//         if (activeAlert) {
...
//         if (isSupervisorMode || driver?.role !== 'driver' || !vehicleId || vehicleId === '110') {
//             return;
//         }

//         try {
//             // Specifically fetch for this vehicle (to=1 is appended in getIncomingMessages if vehicleID is present)
//             const list = await getIncomingMessages(agencyID, vehicleId);

//             if (list.length > 0) {
//                 const latest = list[0];
//                 if (latest.messageID !== lastMessageIdRef.current) {
//                     // If we have a truly new message (and it's not the very first poll result on app start)
...
//         }
//     }, [agencyID, vehicleId, isSupervisorMode, driver?.role]);

//     useEffect(() => {
//         // Only poll if driver role
//         if (isSupervisorMode || driver?.role !== 'driver' || !vehicleId || vehicleId === '') {
...
//             }
//         };
//     }, [vehicleId, isSupervisorMode, driver?.role, pollDriverMessages]);
...
//     if (!ctx) throw new Error('useDriverMessaging must be used within DriverMessagingProvider');
//     return ctx;
// };
